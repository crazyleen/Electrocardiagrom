    ...
    d_timerId(-1)
{
    ...
    setAxisScale(QwtPlot::yLeft, -50.0, 1000.0);
    ...
    d_origin->setLinePen(QPen(Qt::blue, 0.0, Qt::DashLine));
...
void Plot::saveToFile()
{

     CurveData *data = (CurveData *)d_curve->data();

     QString dateString = QDateTime::currentDateTime().toString("yyyyMMddhhmmss");

#if LINUX_EMBEDDED_SYSTEM
     QString ldir = "/mnt/udisk/";
#else
     QString ldir = "D:\\";
#endif
     QString fileName = QFileDialog::getSaveFileName(this,
         tr("Save sample data"), ldir + dateString,
         tr("sample data (*.qvpf);;All Files (*)"));

     if (!fileName.isEmpty() && !fileName.isNull()){
#if LINUX_EMBEDDED_SYSTEM
        fileName = fileName + ".qvpf";
#endif
        data->values().saveToFile(fileName);
     }
}

void Plot::loadFromFile(QString &loadFile)
{
    QString fileName;
     CurveData *data = (CurveData *)d_curve->data();

     if (loadFile.isEmpty() || loadFile.isNull())
     {
#if LINUX_EMBEDDED_SYSTEM
     QString ldir = "/mnt/udisk/";
#else
     ...
     if (!fileName.isEmpty() && !fileName.isNull())
        data->values().loadFromFile(fileName);

     d_interval = QwtInterval(0.0,
         d_interval.width());
     resetViewZome();
}

void Plot::resetViewZome()
{
    setAxisScale(QwtPlot::xBottom, d_interval.minValue(), d_interval.maxValue());
    setAxisScale(QwtPlot::yLeft, -500.0, 1000.0);
    d_origin->setValue(d_interval.minValue() + d_interval.width() / 2.0, 0.0);
    replot();
}

void Plot::restartSample(void)
{


    //d_interval = QwtInterval(0.0, d_interval.width());
...
void Plot::repaintOnLeft(bool repaint)
{
    repaintonleft = repaint;
}
...
void Plot::timerEvent(QTimerEvent *event)
{
    ...
    if ( event->timerId() == d_timerId )
    {
        ...
        if (repaintonleft)
        {
        // start to repaint the curve on the left
